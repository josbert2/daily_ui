...
class inspectTw{
    constructor(elemntID, tailwindConfiJSON){
        this.dev = true
        ...
        this.prefixBreakpoint = [
            // Aquí puedes agregar los prefijos de breakpoint que necesites
        ];
        this.arrayListClassTailwind = {
            'aspectRatio': 'aspect',
            'columns': 'columns',
            'break-after': 'break-after',
            // Aquí puedes agregar más clases de Tailwind que necesites
        };
        ...
        inputBox.onkeyup = (e) => {
           if (e.keyCode != 40 && e.keyCode != 38 && e.keyCode != 13) {
               ...
               if (e.keyCode == 40) {
                   ...
                   var dataSelected = document.querySelectorAll('.select-item')[this.indexSelect].getAttribute('data-value')
                   document.querySelector('#input-tw-search').value = dataSelected
   
                   
                   document.querySelectorAll('.select-item')[this.indexSelect].classList.add('active')
                   document.querySelectorAll('.select-item')[this.indexSelect].scrollIntoView({
                       behavior: 'smooth',
                       block: 'center',
                       inline: 'center'
                   });
                ...
                this.indexSelect--
                   if (this.indexSelect < 0) {
                    this.indexSelect = document.querySelectorAll('.select-item').length - 1
                   }
                   ...
                   document.querySelectorAll('.select-item')[this.indexSelect].click()
               }
    ...
    ColoresHtml(){
        ...
        Object.entries(this.fullConfigTW.theme.colors).forEach(([keys, valueColor]) => {
            var middleColor = this.get500Colores(valueColor)
            ...
            divContainerCategoria.innerHTML = '<div class="container-wrapper-config-colors"><div class="flex items-center px-3"><div style="background:' + middleColor + '" class=" w-4 h-4 rounded-md mr-3"></div><span class="show-element-hover">' + keys + '</span>  <div class="ml-auto">' + this.arrowDown  + '</div></div></div>'
    ...
    getColors(){
        Object.entries(this.fullConfigTW.theme.colors).forEach(([keys, value]) => {
 
            if (typeof value === 'string') {
                this.dataMaster.push('bg-' + keys)
            }
            if (typeof value === 'object') {
                Object.entries(value).forEach(([key, value], index) => {
                    this.dataMaster.push('bg-' + keys + '-' +  key)
                })
            }
        })
    ...
    definedScreen(){
        Object.entries(this.fullConfigTW.theme.screens).forEach(([key, value]) => {
            const screen = key
            const screenValue = value
            this.prefixBreakpoint.push(screen)
        })
    }
    addEventInit(){
        ...
        this.addEvent(document, 'click', '.select-item', function(e) {
            ...
            this.prefixBreakpoint.forEach((prefix) => {
                 
                 if (selectData.indexOf(prefix + ':') >= 0) {
                     var edited = "{";
                     for (var i = 0; i < dataColor.length; i++) {
                         edited += '"'+this.prefixBreakpoint[i]+'":"'+dataColor[i]+'",';
                     }
        ...
        this.addEvent(document, 'click', '.copy-class', function(){
            ...
            newClass = newClass.replace('undefined', '');

            // Copiar la clase al portapapeles
    ...
    preinit(){
        ...
        mainMasterData(resolveConfig(this.fullConfigTW), this.dataMaster)
...
// Iniciamos el objeto
const inspect = new inspectTw('app', {})
inspect.enable()